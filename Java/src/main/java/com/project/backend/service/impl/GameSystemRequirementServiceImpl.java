...
public class GameSystemRequirementServiceImpl implements GameSystemRequirementService {
    ...
    @Override
    public List<GameSystemRequirementDto> listGameSystemRequirementByGame(Long gameId) {
        List<GameSystemRequirement> gameSystemRequirements = repository.findByGame(gameId);
        return gameSystemRequirements.stream().map((gameSystemRequirement) -> GameSystemrequirementMapper.mapToGameSystemRequirementDto(gameSystemRequirement)).collect(Collectors.toList());
    }
